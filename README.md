...
- **Sorting by time**: Tasks are sorted chronologically using a lambda function on the HH:MM time attribute.
- **Priority-based scheduling**: Tasks are sorted by priority first (urgent → high → medium → low), then by time.
